...
abstract contract TakeManyRunSwapsAndTransferManyCompressed is Decompressor, SwapAdapter {
  ...
  function uncompressAndExecute(bytes memory _tx, uint256 length) public payable {
    // decompress(_tx, length);
    // Address.functionDelegateCall(address(this), decompress(_tx, length));
  }
